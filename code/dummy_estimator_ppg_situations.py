...
def get_paths(basePath, split):
    test_paths = []
    train_paths = []
    paths = list(Path(basePath).rglob("*dataFile.hdf5"))
    for val in paths:
        if val.parts[4][1:]==split:
            test_paths.append(str(val))
        else:
            train_paths.apeend(str(val))
    return test_paths
...
if __name__ == "__main__":
    ...
    if dataset != "PPG":
        ...
        path_of_video_train = list(map(str,path.rglob("*data.hdf5")))
        path_of_video_test  = list(map(str,path.rglob("*data.hdf5")))
